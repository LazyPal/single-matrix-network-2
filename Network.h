...
public:
	...
	Network(int inputs, int interneurons, int outputs, char * out_file_name ); // construct a blank network to user specified size and write it to a file for later editing
	virtual ~Network();
...
public:
	...
	double squashingFunction(double value, double max, double slope, double xoffset);
	...
	void writeNetworkInputToFile(  char * file_name );
	...
	void writeNetworkSquashedOutputStateToFile(char * file_name);
	...
	void printNetworkSquashedOutputState(void);
